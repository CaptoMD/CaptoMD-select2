describe('Multiple Int Array', function () {
  var TestPage = function () {
    this.textInput = element(by.id('multipleIntText'));
    this.select = element(by.css('#multipleIntSelect2 + .select2-container'));
    this.selectEntry = element(by.css('.select2-search__field'));

    this.get = function () {
      browser.get('http://localhost:9000/test/fixtures/multipleIntArray.html');
    };
    ...
    this.clickChange = function () {
      element(by.id('multipleIntChange')).click();
    };
